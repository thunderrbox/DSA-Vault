...
export default async function DifficultyDetailPage({ params }) {
  const { level } = await params;
  const capitalizedLevel = level.charAt(0).toUpperCase() + level.slice(1).toLowerCase();

  if (!["Easy", "Medium", "Hard"].includes(capitalizedLevel)) {
    notFound();
  }

  // Fetch problems by difficulty
  const problems = await db.problem.findMany({
    where: { difficulty: capitalizedLevel },
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col gap-6">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight">Difficulty: {capitalizedLevel}</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Displaying all {problems.length} solved {capitalizedLevel} challenges.
          </p>
        </div>
        ...
        {problems.length > 0 ? (
          ...
          <p className="text-slate-400 text-center py-12">No problems solved under this difficulty yet.</p>
        )}
      </main>
